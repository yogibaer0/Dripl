// server.js (dripl)
// ESM-friendly, static hosting + SPA fallback, proxy/cookie rotation, clean error mapping
...
const YT_CLIENT = process.env.YT_CLIENT || ""; // set to "android" if extractor errors hit

// ---- inputs: cookies & proxies ----
...
function runYtDlp(opts) {
  return new Promise((resolve) => {
    ...
    const t = setTimeout(() => { try { child.kill("SIGKILL"); } catch {} ; resolve({ ok: false, code: "timeout", out, err }); }, YTDLP_TIMEOUT_MS);
...
// SPA fallback AFTER API routes so it doesn't swallow them
